...
public class ProdutoNfeRepo extends DataRepository<ProdutoNfe, Integer> {
    ...
    public ProdutoNfe produtoNfe(Long codItem, Long codEmpresa) {
        ProdutoNfe item = nativeFind(query -> {
            ...
            query.add("             WHEN only_numbers(e.cfop) ILIKE '5%' ");
            query.add("                 THEN only_numbers(e.cfop) ");
            query.add("             ELSE null END ");
            query.add("   FROM produto_cfop e ");
            query.add("   WHERE e.produto = a.grid ");
            query.add("   AND only_numbers(cfop) ILIKE '5%' LIMIT 1) AS cod_nop_de_ent, ");
            query.add("  (SELECT CASE ");
            query.add("             WHEN only_numbers(e.cfop) ILIKE '6%' ");
            query.add("                 THEN only_numbers(e.cfop) ");
            query.add("             ELSE null END ");
            query.add("   FROM produto_cfop e ");
            query.add("   WHERE e.produto = a.grid ");
            query.add("   AND only_numbers(cfop) ILIKE '6%' LIMIT 1) AS cod_nop_fe_ent, ");
        ...
        if ((produto == null) && Strings.isNonEmpty(eanTrib)) {
            ...
            if (produto == null) {
                produto = nativeFind(query -> {
                    ...
                    query.add("  only_numbers(a.codigo_ncm) AS cod_ncm, ");
                    query.add("  (SELECT CASE ");
                    query.add("             WHEN only_numbers(q.cfop) ILIKE '5%' ");
                    query.add("                 THEN only_numbers(q.cfop) ");
                    query.add("             ELSE null END ");
                    query.add("   FROM produto_cfop q ");
                    query.add("   WHERE q.produto = a.grid ");
                    query.add("   AND only_numbers(cfop) ILIKE '5%' LIMIT 1) AS cod_nop_de_ent, ");
                    query.add("  (SELECT CASE ");
                    query.add("             WHEN only_numbers(q.cfop) ILIKE '6%' ");
                    query.add("                 THEN only_numbers(q.cfop) ");
                    query.add("             ELSE null END ");
                    query.add("   FROM produto_cfop q ");
                    query.add("   WHERE q.produto = a.grid ");
