...
public class ProdutoNfeRepo extends DataRepository<ProdutoNfe, Integer> {
    ...
    public ProdutoNfe produtoNfe(Long codItem, Long codEmpresa) {
        ProdutoNfe item = nativeFind(query -> {
            ...
            query.add("  c.grid AS cod_tipo_despesa, ");
            query.add("  only_numbers(a.codigo_ncm) AS cod_ncm, ");
            query.add("  CASE WHEN only_numbers(e.cfop) ILIKE '5%' THEN only_numbers(e.cfop) ELSE null END AS cod_nop_de_ent, ");
            query.add("  CASE WHEN only_numbers(e.cfop) ILIKE '6%' THEN only_numbers(e.cfop) ELSE null END AS cod_nop_fe_ent, ");
            ...
            query.add("LEFT JOIN conta c on (a.conta_usoconsumo = c.codigo) ");
            query.add("INNER JOIN nfast_unidade b ON (b.sgl_unidade = a.unid_med) ");
            query.add("LEFT JOIN produto_cfop e ON (e.produto = a.grid ) ");
    ...
    public List<ProdutoNfe> produtoNfeList(Long codEmpresa, String filtro, Integer limit, Integer offset) {
    ...
    public ProdutoNfe produtoNfeCodigo(String cprod, String cean, String ceanTrib, Integer codFornecedor, Long codEmpresa) {
        ...
        if (Numbers.isNonEmpty(codFornecedor)) {
            produto = nativeFind(query -> {
                query.add("SELECT ");
                query.add("  a.codigo AS cod_item, ");
                query.add("  normali(a.nome) AS des_item, ");
                //uery.add("  a.nome AS des_item, ");
                query.add("  a.codigo_barra AS cod_barra, ");
                query.add("  CASE WHEN a.flag = 'A' THEN 'S' ELSE 'N' END AS ind_item_ativo, ");
                ...
                query.add("  1 AS qtd_unidade_agrupamento_2, ");
                query.add("  i.grid AS cod_tipo_despesa, ");
                query.add("  only_numbers(a.codigo_ncm) AS cod_ncm, ");
                query.add("  CASE WHEN only_numbers(e.cfop) ILIKE '5%' THEN only_numbers(e.cfop) ELSE null END AS cod_nop_de_ent, ");
                query.add("  CASE WHEN only_numbers(e.cfop) ILIKE '6%' THEN only_numbers(e.cfop) ELSE null END AS cod_nop_fe_ent, ");
                query.add("  null AS cod_nop_de_tra, ");
                ...
                query.add("  null AS cod_cofins ");
                query.add("FROM produto a ");
                query.add("LEFT JOIN conta i on (a.conta_usoconsumo = i.codigo) ");
        ...
        if ((produto == null) && Strings.isNonEmpty(eanTrib)) {
            ...
            if (produto == null) {
                produto = nativeFind(query -> {
                    ...
                    query.add("  i.grid AS cod_tipo_despesa, ");
                    query.add("  only_numbers(a.codigo_ncm) AS cod_ncm, ");
                    query.add("  CASE WHEN only_numbers(q.cfop) ILIKE '5%' THEN only_numbers(q.cfop) ELSE null END AS cod_nop_de_ent, ");
                    query.add("  CASE WHEN only_numbers(q.cfop) ILIKE '6%' THEN only_numbers(q.cfop) ELSE null END AS cod_nop_fe_ent, ");
                    query.add("  null AS cod_nop_de_tra, ");
                    query.add("  null AS cod_nop_fe_tra, ");
                    query.add("  null AS cod_ipi, ");
                    query.add("  null AS cod_pis, ");
                    ...
                    query.add("INNER JOIN nfast_unidade h ON (h.sgl_unidade = a.unid_med) ");
                    query.add("LEFT JOIN produto_cfop q ON (q.produto = a.grid ) ");
                    query.add("WHERE CASE WHEN a.flag = 'A' THEN 'S' ELSE 'N' END = 'S' ");
    ...
    public ProdutoFor getProdutoFor(Long codItem, Integer codFornecedor, String codItemFornecedor) {
